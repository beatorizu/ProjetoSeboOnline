
<div class="col-xs-5 col-sm-5 col-md-9" id="content">
	<div class="panel panel-primary" style="margin: 10px;"
		id="div-editarmeuperfil-meuperfil">
		<div class="panel-heading">
			<h3 class="panel-title">Editar meu Perfil</h3>
		</div>
		<div class="panel-body">
			<form class="form-horizontal">
				<div class="container" style="width: 100%;">
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Nome: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-nome"
								placeholder="Digite seu nome...">
						</div>
						</br>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Sobrenome: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-sobrenome"
								placeholder="Digite seu sobrenome...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Email: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-email"
								placeholder="Digite seu email...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Senha: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-senha"
								placeholder="Digite sua senha...">
						</div>
					</div>

					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> RG: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-rg"
								placeholder="Digite seu RG...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> CPF: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-cpf"
								placeholder="Digite seu CPF...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Telefone: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-telefone"
								placeholder="Digite seu telefone...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Rua: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-rua"
								placeholder="Digite sua rua...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Bairro: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-bairro"
								placeholder="Digite seu bairro...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Cidade: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-cidade"
								placeholder="Digite sua cidade...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-3">
							<label class="col-sm-3 control-label"> Estado: </label>
						</div>
						<div class="col-md-9">
							<input type="text" class="form-control" id="in-estado"
								placeholder="Digite seu estado...">
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<button type="submit" class="btn btn-primary" style="width: 50%;">Salvar</button>
						</div>
					</div>
				</div>

			</form>
		</div>
	</div>
</div>

